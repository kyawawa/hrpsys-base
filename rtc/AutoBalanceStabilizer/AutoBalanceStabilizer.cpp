...
void AutoBalanceStabilizer::updateBodyParams()
{
    ...
    const hrp::Sensor* const sen = m_act_robot->sensor<hrp::RateGyroSensor>("gyrometer");
    const hrp::Matrix33 senR = sen->link->R * sen->localR;
    m_act_robot->rootLink()->p = ref_base_pos;
    m_act_robot->rootLink()->R = hrp::rotFromRpy(act_rpy) * (senR.transpose() * m_act_robot->rootLink()->R);
